...
普通玩家不需要部署服务器。请从
[v2.1.1-alpha.4 预发行页面](https://github.com/EytleBB/CS-Scout/releases/tag/v2.1.1-alpha.4) 下载
`CS-Scout-Windows-x64-v2.1.1-alpha.4.zip`，完整解压后依次双击：

1. `windows\Install-CS-Scout.cmd`：自动准备独立 Python 环境并安装固定版本依赖；
...
| `GET` | `/healthz` | 公开存活探针 |
| `GET` | `/readyz` | 公开就绪探针，检查分析授权方式、地图和运行目录 |
...
查看页面、实时进度和已有分析结果不需要密钥。服务器部署的 `POST /api/analyze` 使用
`Authorization: Bearer <分析密钥>`，同时继续兼容旧客户端 JSON 中的 `key` 字段。Windows
玩家版启用仅限回环地址的本地模式，不显示密钥框，也不要求密钥。
