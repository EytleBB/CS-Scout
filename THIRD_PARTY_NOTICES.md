...
本文件适用于 CS-Scout v2.1.1-alpha.1，用于说明项目与第三方平台、软件和素材之间的关系。它只提供事实性信息，不构成法律意见，也不替代各上游项目的完整许可证或服务条款。

CS-Scout 项目原创源代码采用仓库根目录 `LICENSE` 中的 MIT License。下述第三方组件、
商标和素材说明不因项目主许可证而被重新授权。

本声明和 MIT 主许可证针对 v2.1.1-alpha.1 的当前源代码快照与发布包。Git 历史中已经删除的
第三方文件仍遵循其原先标示的单独条款，不因本次发布而被重新授权，也不属于 v2.1.1-alpha.1
玩家发布包。

## 非官方关系与商标

CS-Scout 是独立开发的社区工具，不是 Valve Corporation、5E 对战平台、完美世界竞技平台、Awpy 或下列第三方依赖项目的官方产品，也未得到这些主体的赞助或背书。

Counter-Strike、Counter-Strike 2、CS2、Steam 及其相关名称和标识的权利归 Valve 及相应权利人所有。5E 与完美世界竞技平台的名称、标识及平台内容分别归其相应权利人所有。本项目提及这些名称，仅用于说明兼容对象、数据来源或功能用途，不表示任何官方关联。Windows 发布包不会分发完美平台客户端或其 `PvpAlive.dll`；完美模式只调用用户电脑上已经安装的官方组件。
...
Windows 发布包中的雷达地图通过 Awpy 工具链准备，再由本项目的 `server/setup_maps.py` 转换为运行时所需的地图目录和坐标数据。项目维护者已确认：v2.1.1-alpha.1 公开发布包内所含雷达图、现有 SVG 图标和 Logo 可由本项目公开使用。
...
下表列出 v2.1.1-alpha.1 的直接依赖及其上游声明的许可证。Windows 安装脚本通过 `pip` 下载运行依赖；Linux 部署另外使用 Gunicorn；Awpy 只用于准备地图。请以链接中的上游许可证原文为准。

| 组件 | v2.1.1-alpha.1 固定版本 | 用途 | 上游许可证 | 官方链接 |
| --- | ---: | --- | --- | --- |
...
| websocket-client | 1.8.0 | 连接本机 5E 客户端的 CDP WebSocket | Apache-2.0 | <https://github.com/websocket-client/websocket-client> |
...
CS-Scout 会访问 5E 或完美世界竞技平台相关服务以查找比赛和下载 Demo。使用者应在自己有权访问数据的范围内使用本工具，并自行阅读和遵守当时适用的平台服务条款、平台规则及当地法律。第三方接口、页面和条款可能变化；本项目不保证第三方服务持续可用或永久兼容。完美模式使用当前 Windows 登录会话，只在进程内存中处理访问凭据；不得公开 Token、带签名下载地址或客户端日志。
