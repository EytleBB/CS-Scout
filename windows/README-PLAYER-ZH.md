...
这个版本让每位玩家在自己的 Windows 电脑上运行 CS-Scout。程序默认只监听
`127.0.0.1:5000`，局域网和互联网中的其他设备无法连接，不需要开放防火墙端口。
...
1. 在 GitHub Releases 页面下载名称类似
   `CS-Scout-Windows-x64-v2.0.0.zip` 的 Windows 发布包。
2. 同时下载 `SHA256SUMS.txt`，在 ZIP 所在目录运行
   `Get-FileHash -Algorithm SHA256 .\CS-Scout-Windows-x64-v2.0.0.zip`，确认结果与
...
2. 程序启动后会自动打开 `http://127.0.0.1:5000`。
...
先关闭之前打开的 CS-Scout 黑色窗口。若仍然报错，说明另一个程序正在使用端口 5000；
关闭那个程序后再启动。脚本不会自动结束不认识的进程。

### 网页没有自动打开

保持启动窗口打开，手动访问：<http://127.0.0.1:5000>。
